...
public class MainMenuTest {
    ...
    @Test
    public void testMainMenuInit() throws SlickException {
    	...
    	assertThat("Amount of buttons should be like expected", screen.getButtons().size(), equalTo(7));
    	assertThat("Amount of textfields should be like expected", screen.getTextFields().size(), equalTo(0));
    }
    
    @Test
    public void testMainMenuID() throws SlickException {
    	screen = new MainMenu();    	
    	int id = screen.getID();
    	assertThat("ID should be as expected", id, equalTo(GameState.MAIN_MENU));
    }
}