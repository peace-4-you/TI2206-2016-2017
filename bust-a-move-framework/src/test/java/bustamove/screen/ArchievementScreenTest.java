 ...
 * @version 03.11.2016
 */
public class ArchievementScreenTest {
	@Mock 
    ...
    @Test
    public void testScreenInit() throws SlickException {
    	when(game.getWidth()).thenReturn(500);
    	when(font.getWidth("")).thenReturn(50);
    	screen = new AchievementsScreen(statistics);    	
    ...
    @Test
    public void testScreenID() throws SlickException {
    	screen = new AchievementsScreen(statistics);
    	int id = screen.getID();
    	assertThat("ID should be as expected", id, equalTo(GameState.ACHIEVEMENTS_SCREEN));
    }
}